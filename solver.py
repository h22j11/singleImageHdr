...
class Solver(object):
    ...
    def build_model(self):
        self.gen_up = Generator_up(3, 32, 5, 1, self.length - 1).to('cpu')
        self.gen_down = Generator_down(3, 32, 5, 1, self.length - 1).to('cpu')

        self.struct_up = Structure_up(2, 16, 4, 1, self.length - 1).to('cpu')
        self.struct_down = Structure_down(2, 16, 4, 1, self.length - 1).to('cpu')

        self.combine_up = CombineNet_up(7, 16, 5, 1).to('cpu')
        self.combine_down = CombineNet_down(7, 16, 5, 1).to('cpu')

        self.hdrlayer_pred = hdrlayer(lin_type='LUT', method='Debevec', device='cpu')
        self.hdrlayer_gt = hdrlayer(lin_type='LUT', method='Debevec', device='cpu')
