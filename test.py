...
class test_HDR():
    ...
    def build_model(self):
        ...
        self.gen_up.load_state_dict(torch.load(gen_up_path, map_location=torch.device('cpu')))
        self.gen_down.load_state_dict(torch.load(gen_down_path, map_location=torch.device('cpu')))
        self.struct_up.load_state_dict(torch.load(struct_up_path, map_location=torch.device('cpu')))
        self.struct_down.load_state_dict(torch.load(struct_down_path, map_location=torch.device('cpu')))
        self.combine_up.load_state_dict(torch.load(combine_up_path, map_location=torch.device('cpu')))
        self.combine_down.load_state_dict(torch.load(combine_down_path, map_location=torch.device('cpu')))
